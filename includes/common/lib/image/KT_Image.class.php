...
class KT_image {
	...
	function resize_gd($sourceFileName, $folder, $destinationFileName, $newWidth, $newHeight, $keepProportion)
	{
		...
		/* ADD FIX FOR PNG TRANSPARENCY WHEN RESISIZING */
	...
	function getLibraryError($library) {
		return KT_getResource('PHP_IMAGE_LIBRARY_SUPPORT', 'Image', array($library));
	}
	...
	function checkFolder($path, $right, $from) {
		...
		if (preg_match("/\./ims",$path)) {
			//(C NIELD FIX - THIS LINE IS OLD CODE - NEW LINE IS BELOW)   $arr = split("[/\]", $path);
			$arr = preg_split("#[\\/]#", $path);
