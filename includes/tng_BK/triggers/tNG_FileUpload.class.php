...
class tNG_FileUpload{
	...
	function Execute() {
		...
		//security
		if (strpos($this->dynamicFolder, '{') !== false) {
			return new tNG_error('PHP_UPLOAD_CHECK_FOLDER_ERROR', array(), array($this->dynamicFolder));
		}
		$pos = strpos($this->folder, '{');
		if ($pos !== false) {
			$folder = KT_realpath(substr($this->folder, 0, $pos), true);
			$renameFolder = KT_DynamicData(substr($this->folder, $pos), $this->tNG, '', true);
			if (substr($renameFolder, -1, 1) != '/' ||  substr($renameFolder, -1, 1) != '\\') {
				$renameFolder .= '/';
			}
		} else {
			$folder = KT_realpath($this->folder, true);
			$renameFolder = '';
		}		
		if (substr(KT_realpath($folder . $renameFolder . $saveFileName, false), 0, strlen($folder)) != $folder) {
			return new tNG_error("FOLDER_DEL_SECURITY_ERROR", array(), array($folder . $renameFolder . $saveFileName, $folder));
		}
