 ...
 * @access public
 */
//(C NIELD FIX - THIS LINE IS OLD CODE - NEW LINE IS BELOW)   function Trigger_Default_FormValidation($tNG, &$uniVal) {
...
function Trigger_Login_AutoLogin($tNG)
{
    ...
    if ($tNG->loginType != 'cookie') {
        ...
        if (isset($_COOKIE['kt_login_id']) && isset($_COOKIE['kt_login_test'])) {
            ...
            setcookie("kt_login_id", "", $cookie_timeout, $cookie_path, "", TRUE, TRUE);
            setcookie("kt_login_test", "", $cookie_timeout, $cookie_path, "", TRUE, TRUE);
    ...
    if (isset($tNG->columns['kt_login_rememberme']) && $tNG->getColumnValue('kt_login_rememberme') != "") {
        ...
        setcookie("kt_login_id", $tNG->getColumnValue('kt_login_id'), $cookie_timeout, $cookie_path, "", TRUE, TRUE);
        $kt_test = tNG_encryptString($tNG->getColumnValue('kt_login_password_db'));
        setcookie("kt_login_test", $kt_test, $cookie_timeout, $cookie_path, "", TRUE, TRUE);
    }
    return null;
}
...
function Trigger_Registration_CheckPassword($tNG)
{
    ...
    if (!isset($tNG->columns[$password_field])) {
        $password = tNG_generateRandomString(8);
...
function Trigger_ForgotPassword_CheckEmail($tNG)
{
    ...
    if ($GLOBALS['tNG_login_config']['password_encrypt'] == "true") {
        $tNG->kt_login_password = tNG_generateRandomString(8);
