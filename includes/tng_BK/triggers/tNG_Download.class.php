...
class tNG_Download {
	 ...
	 * @access public
	 */
     public function __construct($relPath, $reference) {
   ...
   /**
    ...
    function incrementCounter() {
		...
		if ($this->table != '' && count($this->pk)>0 && $this->counterField != '') {
			...
			if ($ret === false) {
				$this->setError(tNG_error('INCREMENTER_ERROR', array(), array($this->conn->ErrorMsg(), $sql)));
				return;
			}
    	}
    ...
    function getDownloadLink() {
    	if ($this->preparedFolder == '') {
			...
			$downloadFolder = KT_realpath($this->folder, true);
			$this->preparedFolder = $this->relPath . substr($downloadFolder, strlen($siteroot));
    	}
    	//security
