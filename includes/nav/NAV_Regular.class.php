	...
	class NAV_Regular {
		...
		function getShowAllLink() {
			...
			if (isset($_GET[$show_all_reference])) {
				$url = htmlspecialchars(KT_addReplaceParam(KT_getFullUri(), $show_all_reference));
			} else {
				$url = htmlspecialchars(KT_addReplaceParam(KT_getFullUri(), $show_all_reference, "1"));
			}
			$url = htmlspecialchars(KT_addReplaceParam($url, 'pageNum_' . $this->rsName));
			$url = htmlspecialchars(KT_addReplaceParam($url, 'totalRows_' . $this->rsName));
