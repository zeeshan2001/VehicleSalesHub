...
class MX_RssWriter {
	...
	function MX_RssWriter() {
		$this->supported=array(
			'channel' => array('title','link','description', 'pubDate','copyright', 'lastBuildDate', 'image_url', 'image_title', 'image_link'), 
			'item' => array('title', 'description', 'link', 'author', 'pubDate', 'category', 'source', 'source_url'));	
	...
	function setChannelProperty($key, $value) {
		if (in_array($key, $this->supported['channel']) === false) {
			die("<strong>MX_RssWriter Error</strong><br/>Property '$key' is not suported for 'channel'<br/>\n");
		}
	...
	function setItemProperty($key, $value) {
		if (in_array($key, $this->supported['item']) === false) {
			die("<strong>MX_RssWriter Error</strong><br/>Property '$key' is not suported for 'item' <br/>\n");
		}
	...
	function convertEncoding($str) {
		...
		if ($this->encoding == '') {
			...
			if (!function_exists('mb_detect_encoding') || !function_exists('mb_convert_encoding')) {
				die("<strong>MX_RssWriter Error</strong><br/>Please enable 'mbstring' extension from 'php.ini', so the output can be converted to 'UTF-8' encoding,<br/>or use the \"setRssEncoding(\$encoding);\" method of the MX_RssWriter class. Check the documentation for further information.<br/><strong>Example:</strong><br/>\$MXRss->setRssEncoding('ISO-8859-1');");
			}
